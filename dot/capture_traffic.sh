...
if [[ -z "${SCREENSHOT_SH_LOADED:-}" ]]; then
    ...
    parse_capture_args() {
        if [[ $# -lt 4 ]]; then
            echo "Usage: $0 <src_ip> <dst_ip> <src_port> <dst_port> [-t <seconds>] [-m <messages>] [--interface <iface>]" >&2
            echo "Example: $0 192.168.1.1 192.168.1.2 5000 5001 -t 10 --interface eth0" >&2
            exit 1
        fi
    ...
    capture_traffic() {
        ...
        if ! tshark -i "${CAPTURE_INTERFACE}" \
            ...
            echo "Error: Failed to capture traffic or no packets matched the criteria." >&2
            return 1
        fi
    }
fi
